...
项目现在只有一个画布。后续画布开发、修复和验收统一指当前 `/canvas` 画布运行链路，不再使用两套画布的并列叫法。历史上的独立重建方案已废止；2026-07-27 用户批准 ADR-0005（`docs/adr/0005-infinite-canvas-atomic-replacement.md`）后，Infinite Canvas 候选改造只允许按该 ADR 在隔离候选工作树进行，执行依据为 `docs/plans/2026-07-27-infinite-canvas-staged-replacement.md`，不恢复已废止的 Vue Flow 重建方案。
...
- 禁止在生产长期并列两套画布，生产始终只有一个 `/canvas` 用户入口。
- Infinite Canvas 候选改造只允许按 ADR-0005（`docs/adr/0005-infinite-canvas-atomic-replacement.md`）在独立工作树 `F:\dianshang-worktrees\infinite-canvas-candidate`、独立端口 3466 和隔离数据中进行；候选未过人工验收门禁前，当前 `/canvas` 仍是唯一生产事实，正式 3456 不受影响。
- 除上述 ADR-0005 隔离候选外，禁止继续把 Infinite-Canvas 节点体系迁入当前项目主线。
- 画布如有问题，就在当前画布运行链路中做最小修复，并优先保持当前可运行版本。
- 真实生图、图库、历史、本地保存等能力，仍按当前画布现有接口验收。
- 2026-06-26 独立重建方案继续仅作历史记录、不恢复；后续任何超出 ADR-0005 范围的新画布立项，必须重新写 ADR、验收标准和人工确认。
