...
- Docker、`127.0.0.1:3456` 和生产数据库在架构改造与假 Provider 验收阶段均未操作。2026-07-23 后续经用户单独确认，只在 `127.0.0.1:3458` 完成一笔真实 Lingsuan 付费验证；3456 仍是旧版本对照，不能把当前候选当作线上已生效。

## 2026-07-23 稳定生图自审与压力复核

- 独立标准审查和规格审查发现并修复 5 个候选缺陷：调度器满载时微任务忙重试、非瞬态错误未打断熔断连续计数、运行中取消未标记上游计费歧义、输入目录清理可能删除活动任务，以及图片 HTTPS 直连没有复用独立 keep-alive 池。后续真实网络复核又证明宿主机仅 IPv6 稳定、Docker 仅 IPv4 可用：宿主机强制 IPv4 为 0/10、强制 IPv6 为 10/10；Docker 强制 IPv4 为 10/10。直连池因此按运行环境默认 Windows/IPv6、Linux/IPv4，并允许 `PROVIDER_IMAGE_IP_FAMILY=4|6|auto` 显式覆盖，禁止无条件硬编码同一地址族。部分成功响应现包含 `warnings`，后台补充熔断线路、耗时、上游账单核对提示和进程内存摘要。
- 一次性假 Provider 全链路覆盖 50 个同键并发请求、30 个满队列任务、第 31 个任务的 429/Retry-After、3 个独立失败域全局并发、超时、断连、400、503、三次连续瞬态失败熔断、恢复、活动目录保护、取消和进程重启。正常验收提交 P95 为 57ms，同失败域并发 1、全局并发 3，未出现 5xx、负余额、重复预占、丢任务或重启自动重放。
- 20 轮浸泡额外完成 600 个任务。任务队列每轮归零，余额和 Provider 调用次数一致；进程存活堆从约 58MiB 回落到空闲约 45MiB，外部内存从约 8.9MiB 回落到约 5MiB，没有发现 JavaScript 对象或 Buffer 持续泄漏。RSS 因 V8/原生分配高水位从约 147MiB 升至空闲约 285MiB、峰值约 294MiB，未完全归还操作系统；当前测试门槛为 RSS 小于 384MiB、存活堆增长小于 64MiB，生产仍应为单实例预留至少 512MiB 并持续观察。
- 审查确认 `server.js` 使用的是项目依赖 `node-fetch`，其 `agent` 参数有效，并非 Undici 参数误用；本轮新增的是图片专用直连 Agent，不改变文本 Provider Agent。远端历史中“先实现、后补契约文档”的提交顺序无法在不强推改写历史的情况下修正，已作为流程偏差记录，不改写已推送历史。
- 中转无费用反馈环进一步覆盖全新 TLS 和 multipart：宿主机显式 IPv6 20/20、Docker 显式 IPv4 20/20 完成全新 TLS；两边各 5 次无 Key、1×1 PNG `/v1/images/edits` 均到达中转鉴权层并返回预期 401，没有 timeout/reset。3458 当前显式使用 `PROVIDER_IMAGE_IP_FAMILY=6`；未来 Linux/Docker 候选默认使用 4。该结果只证明连接与小体积上传，不证明带 Key 的大图生成阶段不会发生上游 5xx/reset。
- Packy 无 Key全新 TLS 仍观察到间歇抖动：宿主机 IPv6 首轮 9/10、复轮 20/20，Docker IPv4 首轮 8/10、复轮 18/20；失败均为 5 秒建连超时。HEAD 响应由上游主动关闭连接，探针无法复用 socket，因此结果代表最差的新连接场景。结论不是“连不上问题已消失”，而是灵算地址族错选已修复、Packy/公网新建连接仍可能偶发失败；任务队列、熔断和不自动重放只能控制影响与费用，不能消除外部网络故障。
- Docker/3456 生命周期和配置、生产数据库均未变更；无 Key 到站探针之后，用户另行确认了一笔 3458 真实 Lingsuan 大图验证，结果见下一节。本节其余结论只说明候选源码及一次性 3458/随机端口测试结果。

## 2026-07-23 Lingsuan 单笔真实大图验证

- 用户明确授权后，仅提交一次 `task_mrx51tla21090c35`，幂等键为 `codex-lingsuan-large-20260723-1784788600051`；没有提交第二个任务，也没有跨线路重放。
- 输入为人工检查过的 2880×2880 JPEG，712,420 字节；任务入口成功将它落到独立输入目录。请求使用 `gpt-image-2`、`lingsuan-images`、`/v1/images/edits`、`image[]`、`high` 和 `2880x2880`。
- 提交接口 35ms 返回 202；Lingsuan 通过 `https-ipv6-pool` 返回 HTTP 200。唯一 Provider attempt 耗时 74,945ms，`preTlsRetryCount=0`，证明本次大体积 multipart、TLS、鉴权、上传和上游生成完整成功，未发生连接重试。
- 任务终态为 `success/done`，账务从 10 点预占转为 10 点结算，独立测试账户余额 40→30；任务、item、attempt 均各 1 条，生成历史新增 1 条，没有重复预占或重复调用。
- 结果落盘为 2880×2880 PNG、7,299,921 字节；文件与通过 3458 HTTP 下载的内容逐字节一致，SHA-256 为 `80ffe511854c936506666a2508301c6d7250dca2d843dd0c4c871a5b7524fe90`。人工查看确认图片可正常解码、构图和中文排版完整可见。
- Lingsuan 图片线路 Key 只从 3456 容器中的已保存路线读取到内存，未输出、未写入源码或 3458 数据库。验证结束后 3458 已恢复 `mock`，3456 保持 `real-provider-ready` 且未停止、重启、重建或修改数据。
- 本次成功证明当前 Windows/IPv6 候选链路可以完成单笔真实大图，但不能推导为公网或上游永不失败，也不能替代 10 用户真实付费并发测试；并发可靠性仍以假 Provider 压测和本地有界调度证据为主。

## 2026-07-23 Lingsuan 5 人突发真实大图复核

- 用户随后明确要求连续测试 5 张。5 个独立测试用户在 85ms 内并发提交 5 个 2880×2880 图生图任务，单笔受理为 58–81ms，全部返回 202；首笔立即运行，其余真实队列位置为 1–4。
- 同一 `lingsuan.top|lingsuan-images` 失败域始终只有 1 个上游请求运行。5 个任务、5 个 item、5 个 Provider attempt 一一对应，没有第 6 笔、没有重放、没有任务丢失，采样和时间区间计算的最大同域并发均为 1。
- 真实结果为 3 成功、2 失败，批次墙钟时间 419,708ms。成功三笔分别耗时 99,165ms、76,441ms、74,100ms，均返回 HTTP 200；失败一笔在 44,772ms 后返回 HTTP 400 与 `skipped_mainline=true`，另一笔在 125,223ms 后返回 HTTP 524。
- 5 笔全部使用 `https-ipv6-pool`，`preTlsRetryCount=0`。因此本轮没有复现 IPv4 错选、TLS 建连失败或 socket reset；两次失败均发生在请求到达中转后的上游处理阶段，说明 Lingsuan 真实 4K 大图仍存在业务路由/超时波动。
- 三张成功结果均为 2880×2880 PNG，大小为 6,823,876、7,094,617、7,362,206 字节；磁盘文件与 3458 HTTP 响应逐字节一致。站内先预占 50 点，成功结算 30 点、失败退款 20 点，所有用户余额非负；HTTP 524 是否仍产生上游费用必须以 Lingsuan 账单为准。
- 真实模式结束后 3458 已恢复 Mock；重启后 5 个任务仍通过 API 完整可读，attempt 仍为 5、非终态为 0。3456 继续健康，Docker 和生产数据库没有停止、重启、重建或写入。
- 本轮证明候选的突发接单、持久队列、同域限流、失败隔离、退款和重启恢复符合设计，但不能把 Lingsuan 描述为稳定：本批真实成功率仅 60%。不应为追求表面成功率自动重放 HTTP 400/524，否则可能造成上游重复计费。

## 2026-07-23 Lingsuan 自适应降载修复（3458 候选）

- `ImageRequestScheduler` 增加同失败域默认 5 秒启动间隔、单错冷却和滑动窗口熔断：`skipped_mainline` 冷却至少 60 秒、HTTP 524 冷却至少 120 秒；最近最多 5 次中出现 2 次瞬态失败即熔断，不要求先积满 5 个样本，同时保留连续 3 次失败门槛。并行上限大于 1 时，较晚完成的成功请求不会提前清除另一请求刚触发的冷却。当前任务失败仍直接终态，不自动重放或跨线路切换。
- Provider HTTP 失败现分类为稳定错误码；只保存脱敏允许列表中的 `CF-Ray`、`Retry-After`、Provider 请求号和 Server。任务及 attempt 同步记录 `providerBillingStatus/upstreamBillingAmbiguous/billingAuditRequired`，把“本地已退款”和“上游是否计费”分开表达。
- 等待冷却的任务返回 `stage=provider_degraded` 和实时 `retryAfterMs`；管理端显示失败域冷却、最近窗口失败数、原因、上游计费状态和诊断请求号。
- 新增 `POST /api/generate/tasks/:id/retry`。只允许所属用户人工重试失败/取消任务；进程中断、运行中取消、上游空结果和结果落盘失败等计费未知状态必须显式确认风险。新任务使用新幂等键，原任务不变；参考图已超过 24 小时保留期、丢失或在检查后被清理器竞态删除时均返回 410。
- HTTP 200 但正文携带 `skipped_mainline=true` 且没有返图时，也按 Lingsuan 路由失败处理并触发 60 秒冷却，不再落入普通空结果；终态任务会保留有效 `retryAfterMs`。
...
## 2026-08-16 生图节点显示排队位置（排队中 · 第 N 位）
...
- 恢复生图节点和配置生图面板的“生成中再次生成”入口；同一节点连续点击会创建多个独立任务，继续复用后端并发排队与幂等链路。
- 前端请求追踪改为同一目标节点保留请求列表，任一任务完成不会提前清除仍在运行任务的节点状态；停止入口仍可一次取消同一运行节点的全部任务。
- 验收：候选前端 typecheck、build、Docker 重建和浏览器 smoke 均通过；3466 镜像为 `sha256:82f27bab9e46851d3ab00e2413feca8d97ac73cc14431e71cc953bad44f65e08`，健康检查与 `/api/health` 通过；正式 3456 未动。
