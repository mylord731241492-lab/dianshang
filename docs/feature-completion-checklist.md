...
| 画布 | 进行中 | 74% | 基础画布、工具栏和项目页可打开；未选本地保存文件夹的预期提示已降级为 warning；API smoke 已覆盖项目创建、workflow 云保存、读取恢复、列表缩略图、更新恢复和删除清理。 | 做浏览器 UI 级新增节点、上传素材、保存按钮和重新打开项目验收。 |
...
| 文档审查 | 进行中 | 94% | 已建立进度报告、功能清单、review log、轻量平台 ADR 和每轮推进复核清单；本轮新增画布保存恢复 API 长流程验收记录。 | 每轮推进持续追加，并按 `docs/iteration-review-checklist.md` 收尾。 |
