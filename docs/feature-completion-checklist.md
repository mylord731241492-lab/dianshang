...
- [x] 用户确认后在 3458 完成单笔真实 Lingsuan 大图验证；唯一 attempt HTTP 200，未重试，账务与结果落盘一致。
- [ ] Docker/3456 候选重建、生产切换和真实付费并发测试仍等待单独确认。
...
- [x] 用户确认费用后完成单次真实 multipart 大图验证：712,420 字节输入、2880×2880 输出、HTTP 200、74,945ms、`preTlsRetryCount=0`。
- [ ] 单次成功不能证明上游生成阶段永不 5xx/reset；5 用户真实突发已执行，但同失败域按设计串行，未放开为 5 路上游同时并发。
- [x] 5 个用户真实突发提交在 85ms 内全部受理；同 Lingsuan 失败域最大并发 1，任务/item/attempt 均恰好 5。
- [x] 三笔 HTTP 200 成功落盘，均为 2880×2880；两笔上游 HTTP 400/524 正确失败并退款，没有自动补发。
- [x] 3458 恢复 Mock 后任务、账务和 attempt 均持久存在；3456 Docker 未动。
- [ ] Lingsuan 本批真实大图成功率仅 60%，仍需上游排查 `skipped_mainline`、524 和失败请求实际计费；不可宣称线路稳定。
...
## 2026-07-23 Lingsuan 自适应降载候选

- [x] 同失败域默认 5 秒启动间隔；测试环境可显式设为 0。
- [x] `skipped_mainline` 单次 60 秒冷却、524 单次 120 秒冷却，不自动重放当前任务。
- [x] 最近最多 5 次中出现 2 次瞬态失败即熔断（无需先积满 5 个样本），并与连续 3 次失败门槛同时保留。
- [x] Provider 失败只持久化脱敏诊断头，并区分本地退款与上游计费未知。
- [x] 冷却任务返回 `provider_degraded/retryAfterMs`，管理端展示失败窗口、原因和计费审计提示。
- [x] 人工重试接口要求计费风险确认，创建新任务且不复用旧幂等键；过期参考图返回 410。
- [x] HTTP 200 内嵌 `skipped_mainline`、并行成功误清冷却、重启/落盘计费歧义和参考图复制竞态均有回归覆盖。
- [x] 架构单测、队列全链路、10 用户故障注入、API smoke、Provider 适配器和前端构建通过。
- [x] 额外 3 轮共 90 个假任务 soak 通过；提交 P95 53.8ms，同域并发 1、三域并发 3。
- [x] 两次真实 4K 请求均被当前 Key 明确拒绝，站内预占 20 点全部退款，没有第三次请求；3458 已恢复 Mock。
- [ ] 更新有效 Lingsuan Key 后仍需另行授权真实成功验证；Docker/3456 发布仍未执行。

## 2026-07-29 Infinite Canvas Agent 功能等价迁移

- [x] 已确认 Task 10 `fee1146` 是业务助手替换，不是原版 Agent 全功能迁移。
- [x] 已建立原版与候选功能差异矩阵、Task 13A 实施清单和自动/人工验收标准：`docs/infinite-canvas-agent-feature-migration.md`。
- [x] 架构已锁定为多人网页版：复用 Infinite Canvas MCP 工具，服务端 Lingsuan Agent Runtime 驱动；不依赖用户本机 Codex、Local URL、Connect token，不使用 `hajimi-website` MCP 代替画布 MCP。
- [x] 服务端 Agent 会话按 `userId + projectId + agentSessionId + browserSessionId` 隔离，并覆盖多用户、多项目标签页、断线、过期和退出登录。
- [x] 画布读取与结构化写操作完成自然语言端到端验收。
- [x] 写操作具备提案、确认、拒绝、稳定调用 ID 和幂等执行。
- [x] 项目、任务、提示词、素材和生图配置具备 Agent 等价工具。
- [x] 会话支持新建、刷新恢复、切换、删除和停止运行。
- [x] 工具结果与脱敏诊断可见，不泄露 Token、API Key、密码或授权头。
- [x] 图片/节点上下文、三模式状态和跨账号数据不串。
- [x] Agent 专项自动化和原 Task 13 全量回归通过。
- [ ] 3466 候选与 3467 原版完成同指令 A/B 人工验收。
- [ ] 用户明确同意重新进入 Task 14；此前 Task 14–17 均保持阻断。

## 2026-07-29 候选画布 Blob 参考图修复

- [x] 复现服务端 `node-fetch` 读取浏览器 `blob:` 返回 500。
- [x] 图片节点、插件节点与失败重试统一在浏览器提交前把参考图水合为 Data URL。
- [x] 前端 API 禁止发送 `blob:`；后端输入层返回 400 + `GENERATION_REFERENCE_BLOB_URL_UNSUPPORTED`。
- [x] 失败重试同时识别旧 `image:` 与当前 `asset:` 存储键。
- [x] Playwright 覆盖 `asset:` 参考图失败节点重试、成功落资产和项目 JSON 无 `blob:`。
- [x] 候选 Docker/API/UI smoke 通过，正式 3456 指纹未变化，无真实 Provider 调用。

## 2026-07-29 图片节点与生图节点

- [x] 只复用现有 Image/Config 节点、画布交互、上传、模型参数和持久任务链路，没有新增第三种节点或第二套实现。
- [x] 图片节点支持选择、拖放和 Ctrl+V 粘贴，只承担图片显示与参考图连接输出。
- [x] 图片节点单击不显示提示词、模型、参数或生成面板；上方图片工具条和原连线能力保留。
- [x] 生图节点接收参考图连接，并在同一节点的横向面板中提供提示词、模型、参数和结果区域。
- [x] 绘图结果不再从 Config 节点散落创建图片节点。
- [x] 4 张结果按 2×2 四宫格展示，可选择其中一张作为节点资源输出。
- [x] 多图结果、选中索引和每张 `storageKey` 可持久化并恢复，项目 JSON 不含 `blob:`。
- [x] 两种节点标题常驻显示，旧默认“生成配置/绘图节点”在界面统一显示并迁移为“生图节点”。
- [x] 连续从工具栏创建两种节点会避让已有节点，不再默认叠在画布中心。
- [x] 创建菜单不显示说明小字，采用紧凑单行布局。
- [x] 画布空白处双击在指针位置打开节点创建列表；右键不打开该列表。
- [x] 创建菜单脱离画布缩放层；58% 与 100% 缩放下实际宽度均为 300px。
- [x] Codex 内置浏览器验证两节点不重叠、图片节点无生图面板、生图节点面板与参考图计数正常。
- [ ] 旧全量 UI runner 的重叠节点连线步骤改为稳定坐标夹具后再跑全站回归。
- [ ] 用户在 3466 人工确认最终节点尺寸、工具布局和视觉细节；真实 Provider 仍需另行授权。
