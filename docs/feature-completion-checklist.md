...
| 测试护栏 | 进行中 | 99% | 已有前端路由 smoke、API smoke、后台 UI smoke；`preflight-check.ps1` 默认改用 disposable API smoke，避免污染人工测试库；新增 `SMOKE_UI=true` 可选开关，一条命令覆盖后台 10 页截图和画布 JSON 导入；父脚本已检查 native/子脚本退出码，避免子脚本失败仍显示通过；新增 `scripts/smoke-admin-write-disposable.ps1`，安全覆盖后台删除/恢复/增删改写入；新增用户中心兑换码 UI smoke、后台删除/恢复 UI smoke、移动端截图 smoke 和图库 UI smoke；图库 UI smoke 覆盖桌面/移动端多图、保存全部链接和空状态恢复。 | 后续把 Docker daemon 启动后的容器验证接入实际服务器验收；Playwright 截图脚本保持串行或拆不同 session。 |
