...
| 画布 | 进行中 | 80% | 基础画布、工具栏和项目页可打开；未选本地保存文件夹的预期提示已降级为 warning；API smoke 已覆盖项目创建、workflow 云保存、读取恢复、列表缩略图、更新恢复和删除清理；新增示例项目摘要恢复守卫，避免刷新时读到空摘要后丢失默认节点。 | 做浏览器刷新同一动态项目、上传素材和重新打开同一项目验收。 |
