...
- 迁移索引当前为 `23 / 0 / 23`：23 个源码入口、0 个旧桥接入口、23 个总入口。
- 后台源码页已统一使用 `AdminSourceSidebar.vue` 共用侧栏，12 个后台入口数量和顺序一致。
- `/admin/settings`、`/admin/api-providers` 和 `/admin/chat-settings` 已进入写入试点；真实 Provider 测试、真实 Key、删除正式数据和外部付费调用仍需单独确认测试数据与回滚方式。
...
| 后台 | `/admin/chat-settings` | `frontend/` | 保存试点 | 已接 Chat 运行时策略、部署状态和无费用内部连接测试；容器密钥只显示配置状态。 |
...
2. 按原值记录、保存、刷新回显、恢复原值的顺序验收系统设置、API 线路和 Chat 设置三个写入试点。
