...
## 2026-07-23 Lingsuan 自适应降载与计费歧义

- 新增同失败域 5 秒启动间隔、`skipped_mainline` 60 秒冷却、524 120 秒冷却，以及最近最多 5 次中出现 2 次瞬态失败即熔断的滑动窗口；无需先积满 5 个样本，没有增加自动重放或跨线路切换。
- Provider 错误改为结构化分类并持久化脱敏 `CF-Ray/Retry-After/requestId/server`。本地退款与上游计费状态分离，失败任务可标记“上游计费未知，需审计”。
- 新增人工重试 API，计费歧义任务必须确认风险后创建新任务；旧任务和旧幂等键不复用，参考图过期返回 410。
...
## 2026-08-03 用户中心严格对齐旧版卡片式（Kimi Code）

- 画布内用户中心弹窗 + 首页右侧用户卡片整体重写，严格复刻旧版：升级按钮、随机头像、预设头像下拉、算力明细展开/收起、兑换码、API 线路选择（状态标签/当前线路/线路列表/模型列表/安全说明）、界面语言、退出登录；移除旧版没有的"生成记录"卡。
- 选线走既有后端：GET /api/user/routes + POST /api/user/preferences/api-route，偏好即时生效于 api-status 与后续生图路由解析。
- CDP 双端截图验收通过（含选线切换实测）。Docker 未动。

## 2026-08-03 上 Docker 前修复包（Kimi Code）

- 五路并行审查（后端安全/API 契约/部署/回归/画布链路）后按"内网 10 人稳定出图"定位修 6 项：草稿 UTC 误删、拖拽请求风暴、SPA 白名单、legacy 同源、进程崩溃兜底、生图轮询 20 分钟。
- 内网可信项不修：reset-password 无验证、验证码明文回传、认证零限流（已知风险，文档留档）。
- CDP 预部署验收 PASS；Docker 未动。待办：git commit 后再 docker build（镜像打工作树）。

## 2026-08-03 首页案例模块移除 + 节点复制图片（Kimi Code）

- 首页提示词案例模块应用户要求整体移除；生图节点右键加「复制当前图片」（剪贴板优先，http 降级下载）。
- 双 CDP 验收 PASS，构建全绿，Docker 未动。

## 2026-08-05 首页登录门修复（Codex）

- 修复 3456 首页登录门输入框无法输入：`n-modal` 焦点陷阱抢焦点，`LoginGateModal.vue` 增加 `:trap-focus="false"`。
- 管理员在用户登录门输入时显示后台入口提示（403 优先服务端 message）。
- 验证通过：构建 + Playwright（登录/注册/弹窗关闭/项目加载）；测试账号已清理。
- 3456 本地 node server 当前入口 `index-B9aQaa_n.js`；改动未提交，生产 Docker 未动。

## 2026-08-05 /canvas 切换为无限画布（Codex）

- 根因：3456 本地 node 进程启动时未设 `CANVAS_RUNTIME`（默认 legacy），`/canvas` 走旧根入口显示旧画布；项目本身没有拉错。
- 已以 `CANVAS_RUNTIME=infinite` 重启 3456，`/canvas` 命中无限画布并完成登录后真实渲染验证（画布库/新建画布/Agent 助手）。
- 启动候选 3456 必须带 `CANVAS_RUNTIME=infinite`；测试账号已清理；生产 Docker 未动。

## 2026-08-06 候选端登录统一为整页登录（Codex，已提交）

- 移除首页强制登录弹窗；受限功能（画布/模板/图库/用户中心）未登录统一跳 `/login?redirect=`；登录后返回原目标。
