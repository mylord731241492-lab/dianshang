...
- 所有 `gpt-image-2` 文生图请求必须走 `callProviderImageGeneration`，由该适配器统一写入 `size`、`quality`、`output_format`、`response_format=url` 和 `n=1`。
- 所有 `gpt-image-2` 图生图 / 图片编辑请求必须走 `callProviderImageEdit`，由该适配器统一写入 multipart 图片、mask、`size`、`quality`、`output_format`、`response_format=url`、`n=1` 和 `input_fidelity=high`。
- 当前已纳入统一适配器的入口：
