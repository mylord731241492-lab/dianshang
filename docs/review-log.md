...
- 规格审查确认活动输入目录可能被 24 小时清理误删、部分成功缺 `warnings`、后台未直接显示熔断和阶段耗时；均已修复。关于 `fetch.agent` 的初步质疑经源码核对被排除：项目调用的是 `node-fetch@3`，不是 Node 全局 Undici fetch；实际缺口是直连图片请求未统一使用连接池，现已使用独立 IPv4 keep-alive Agent，文本链路不受影响。
