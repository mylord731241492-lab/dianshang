...
## 2026-08-06 故障注入复核：瞬时错误下现状编排 vs 请求级重试

### 已验证

- mock 上游注入固定序列（429+Retry-After、500、socket 断连、成功、429、成功），官转路由 baseUrl 热切换到 127.0.0.1:3480（DB 热读，无需重启），测后已恢复 edge.lingsuan.org。
- A（经 3466 现状）：2/6 成功、4/6 失败；429/500/断连均直接 failed；熔断 60s 冷却导致 #3 排队 123s、#5 排队 183s。
- B（独立脚本桥豆式重试）：6/6 成功，4/6 被重试救回（429×2、500、断连），额外 4 次尝试，每张仅多等 1~2s。重试 slot 首次=注入故障、重试=成功，保证同序列可比。

### 结论

- 现状编排对瞬时错误零容忍且会连带放大延迟（熔断 60s）；请求级重试可把成功率从 33% 提到 100%。建议在调度器落地「瞬时错误自动重试（1~2 次、退避 1s→3s、429 按 Retry-After）」。
- 边界：200-空响应（已计费）与幂等键防重放必须保留；500/超时属于模糊计费，重试次数建议 1 次并记录 billingAuditRequired。

### 未覆盖测试

- 未在真实上游故障窗口实测（mock 已验证逻辑）；未测 Packy 线路；未测画布 Agent 文本链路重试。
## 2026-08-06 官转生图「单发 vs 桥豆式重试」对比复核

### 已验证

- 代码审计结论：模板生图每条 item 单次上游调用（callProviderImageGeneration，180s 超时），仅 TLS 握手前断连自动重试 2 次；其余瞬时错误（429/5xx/超时/ECONNRESET）进入调度器熔断/冷却后任务直接 failed 并退款，无请求级重试。画布 Agent 文本链路 callProviderResponses 单次 fetch，120s 超时即 PROVIDER_TIMEOUT，会话置 error。
- 对比实测（2026-08-06，官转 gpt-image-2，edge.lingsuan.org）：A 经 3466 单发 10 张 10 成功（平均 70.3s、排队 24.6s、最快 27s、最慢 149s）；B 直连+重试 10 张 10 成功、全部首试成功、重试救回 0 张（平均 61s、最快 32s、最慢 93s）。21 次调用均 attempt 1 / upstream=200，无瞬时失败。
- 调度器快照：并发 4/4/2、启动间隔 500ms、请求延迟 200ms；熔断 recentAttempts=3、consecutiveFailures=0。

### 新发现风险

- 本次窗口内重试无收益，不能据此证明重试机制无价值：若未来出现 429/断连/超时瞬时错误，单发仍会失败而重试可能救回。真实收益需在故障窗口实测。

### 未覆盖测试

- 未在人为注入 429/断连/超时的场景下做 A/B；未测 Packy 线路与画布 Agent 文本链路的重试对比。

### 下一轮优先级

- 若用户需要，可先在调度器落地「瞬时错误请求级自动重试（1~2 次，退避 1s→3s，429 按 Retry-After 等待）」，再在候选端做故障注入回归。
## 2026-08-06 桥豆式重试落地复核（代码级）

### 已验证

- 用户确认采用桥豆模式后，`server.js` 生图链路落地 `runProviderImageRequestWithRetry`：429/408/5xx/超时/断连自动补发，默认 1 次上限 3 次、退避 1s→2s、429 按 Retry-After 等待上限 60s；200-空响应（已计费）、LINGSUAN_SKIPPED_MAINLINE、524 不重试。生图 Agent 改为按域名 keep-alive 池（maxSockets=domainConcurrency+1）。
- 文本链路 `callProviderResponses` 增加安全重试（默认 0 次，仅画布 Agent 链路启用）；调度器新增 `userConcurrency`（默认 1、上限 5）。
- 回归（mock 序列 429→成功→500→成功→断连→成功，每张首次必错）：改造后 6/6 成功、2~6s/张，mock 日志 12 次上游调用证明重试均发生；改造前同序列 2/6 失败。真实官转冒烟 task_msh7lyce12b6415d success、cost=10。
- 清理：fault_1 用户及 7 条任务/生成/余额日志删除；mock 进程停止；object-storage 与 generation-task-inputs 测试残留删除；`generation-task-inputs/`、`object-storage/` 加入 .gitignore。

### 新发现风险

- 500/超时属于模糊计费，重试 1 次时结果对象已带 `billingAuditRequired` 标记；生产开启重试后需监控该标记出现频次。
- 未在真实上游故障窗口实测重试救回效果（mock 已验证逻辑）；未测 Packy 线路。
...
## 2026-08-06 DS Pro 后端重构 PRD 复核

### 已验证

- PRD 与 ADR-0007（模块边界）、ADR-0008（健康判定 B-）一致；触发条件为 DS Pro 发布 + 用户明确开始。
- M0 准备（契约/smoke 基线）不依赖 DS Pro，可先行；M1-M4 拆分顺序 auth → users → projects → admin 与 ADR-0007 一致。
- 验收标准可量化（server.js ≤1500 行、db.prepare=0、/api 契约等价、单测通过）。

### 新发现风险

- 「DS Pro」指代待用户确认（文档按 DeepSeek Pro 假设）。
- PRD 触发前不做拆分属有意冻结；期间仅可做 M0 文档/基线准备。

### 下一轮优先级

- 用户确认 DS Pro 指代 + M0 是否启动；确认后执行 M0 基线固化。
...
## 2026-07-28 Infinite Canvas 候选 Task 13 复核

- 隔离边界通过：候选只绑定 `127.0.0.1:3466`，数据只写入候选工作树 `.scratch/infinite-canvas-candidate/`，Compose 不包含正式数据卷或正式服务。
- 正式环境保护通过：验收脚本运行前后逐字比较正式容器 ID、镜像 ID、启动时间和健康状态；未执行正式 Compose、重启、重建或数据写入。
- API 契约通过：项目、素材、系统/用户提示词、生成任务、取消/退款/重试、图片工具和跨用户 404 隔离均由候选 Fake Provider/Fake Storage 覆盖。
- UI 阻断项通过：节点创建/拖动/连线/撤销重做/框选/缩放/小地图，素材库分页/搜索/改名/标签/插入/删除，提示词和遮罩编辑，项目保存/刷新/重命名/删除均完成。
- 性能恢复复核通过：10 节点中 9 张图片共享同一 `storageKey` 时只解析一个 Blob URL，图片全部渲染；拖动期间自动保存请求不超过 1 次，项目 JSON 无 Base64/Blob URL。
...
## 2026-08-03 用户中心旧版对齐审查（Kimi Code）

- 旧版功能清单来源：main 分支 `assets/i18n-*.js`（完整 zh-CN 字典）+ 用户提供的旧版截图，逐项核对无遗漏。
- 验收证据：`.scratch/cdp-uc-align.js` 自动断言 15 个必备区块两处全存在；选线点击后"当前线路"切换且跨端（首页→画布）一致，证明偏好持久化链路真实生效而非仅 UI。
- 已知取舍：旧版"升级"按钮对应充值套餐（旧版标注"待接入支付平台"），现实现为提示"可先用兑换码"；生成记录卡按"严格一致"要求移除，入口仍在画布菜单与 /user/records。

## 2026-08-03 用户中心层级重排审查（Kimi Code）

- 层级手法：色彩权重（渐变仅用于主卡）、尺寸权重（余额 28px 为全抽屉最大字）、分组（资产三合一并加分隔带）、弱化（明细默认收起、语言 slim 卡、退出登录文本链）。
- 选择语义：线路行新增单选圆点（radio dot），active 行橙底橙边 + check，避免"列表还是按钮"的歧义。

## 2026-08-03 上 Docker 前全面审查（5 路并行：后端安全/API 契约/部署就绪/用户中心回归/画布链路）
...
## 2026-08-20 画布剪贴板图片回归审查

- 根因：原有 `Ctrl/Cmd+V` 直接依赖 `navigator.clipboard.read()`，内网 HTTP/权限受限时读取失败；同时全局 keydown 提前阻止了原生 paste 事件。
- 修复：无内部节点剪贴板时放行原生粘贴事件，优先消费 `clipboardData.items/files` 的图片，上传完成后创建图片节点；输入框和可编辑区域继续放行浏览器默认粘贴。
- 结论：Docker 内网实测通过，拖拽和内部节点复制路径未改；主工作区、3456 和真实 Provider 未动。

## 2026-08-20 候选画布生图连按恢复审查

- 根因：`db6e7e5` 的连按实现随后被 `02eb0a8` 的“生成中禁点”覆盖，当前 Docker 按最新候选源码构建后表现为旧的禁点按钮。
- 修复：恢复两个生图入口的连按交互，并将 `generationRequestsRef` 改为按目标节点保存请求列表；最后一个同节点任务结束前保持运行态。
- 审查结论：前端 typecheck/build、3466 Docker 健康检查和浏览器连续点击回归通过；测试产生的画布与任务已清理。只涉及候选工作树和 3466，未修改 `F:\dianshang` 主目录、3456 或真实 Provider。
