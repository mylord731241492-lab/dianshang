...
## 2026-06-30 Canvas Chat 对话生图业务线复核

### 已确认

- 用户明确的业务线是：先按顺序上传参考图，再填写提示词需求，先生成可编辑提示词，用户确认后才调用图片模型生图。
...
- 新增 `/api/canvas/generate-prompt`，文本线路真实可用时走 `/responses` 生成提示词；mock 或失败时返回基础可编辑草稿。确认生图继续使用 `/api/generate/tasks`，模型固定为 `gpt-image-2`。
- boundary smoke 已加入新 JS/CSS 资源、未登录 401 边界和管理员登录后的提示词草稿返回形态，避免接口或资源 404 回退。

### 需要继续验证

- 刷新当前画布后，在 `对话` 标签上传两张图并输入需求，确认出现“提示词草稿”卡片和 `确认生图` 按钮，且不再新增 `gpt-5.5` 失败结果卡。
- 用户确认额度后点击 `确认生图`，验证 `/api/generate/tasks` 真实返回 GPT Image 2 结果图；当前补丁先把结果显示在聊天面板，结果自动落成画布图片节点还需后续单独桥接。
