...
export const frontendMigrationRoutes: FrontendMigrationRoute[] = [
  ...
  { path: '/canvas', title: '旧画布', area: 'canvas', status: 'source', note: 'Vue 入口直接跳转旧画布运行时，不再显示迁移中转页。' },
  ...
  { path: '/admin/dashboard', title: '后台控制台', area: 'admin', status: 'source', note: '源码第一版已接入只读 Dashboard 统计。' },
  ...
  { path: '/admin/api-providers', title: 'API 线路管理', area: 'admin', status: 'source', note: '源码第一版已接入只读 API 线路列表与搜索。' },
  ...
  { path: '/admin/settings', title: '系统设置', area: 'admin', status: 'source', note: '源码保存试点已接入低风险设置草稿和保存回显。' }
];
