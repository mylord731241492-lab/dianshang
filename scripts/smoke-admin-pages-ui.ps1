...
$session = $env:SMOKE_ADMIN_PAGES_UI_SESSION
if (-not $session) {
  $session = "admin-pages-ui-smoke-" + [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds()
}

$runnerScript = Join-Path $PSScriptRoot "smoke-admin-pages-ui-runner.js"
...
function Invoke-PlaywrightCli {
  ...
  $stdout = Join-Path ([System.IO.Path]::GetTempPath()) ("dianshang-admin-pages-" + $Step + "-out.log")
  $stderr = Join-Path ([System.IO.Path]::GetTempPath()) ("dianshang-admin-pages-" + $Step + "-err.log")
