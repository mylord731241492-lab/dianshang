...
$port = $env:SMOKE_ADMIN_WRITE_TEMP_PORT
if (-not $port) {
  $port = "4596"
}

$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("dianshang-admin-write-smoke-" + [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
try {
  ...
  if (-not $ready) {
    ...
    throw "Disposable admin write smoke server did not become ready on port $port. $errText"
  }
    ...
    if ($resolvedTempRoot -and $resolvedTempRoot.Path.StartsWith($systemTemp, [System.StringComparison]::OrdinalIgnoreCase)) {
      for ($i = 0; $i -lt 8; $i++) {
        try {
          ...
          if ($i -eq 7) {
            Write-Warning "Could not remove temp admin write smoke directory: $($resolvedTempRoot.Path). You can delete it later."
          } else {
            Start-Sleep -Milliseconds 500
          }
        }
