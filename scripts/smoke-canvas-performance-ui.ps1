...
$openUrl = "$baseUrl$canvasPath${separator}canvas-performance-smoke=open"

$session = $env:SMOKE_CANVAS_PERFORMANCE_UI_SESSION
if (-not $session) {
  $session = "canvas-performance-ui-smoke-" + [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds()
}
...
function Invoke-PlaywrightCli {
  ...
  $stdout = Join-Path ([System.IO.Path]::GetTempPath()) ("dianshang-canvas-performance-ui-" + $Step + "-out.log")
  $stderr = Join-Path ([System.IO.Path]::GetTempPath()) ("dianshang-canvas-performance-ui-" + $Step + "-err.log")
...
$runnerScript = Join-Path $PSScriptRoot "smoke-canvas-performance-ui-runner.js"

Write-Host "Running canvas performance UI smoke checks with Playwright session: $session"
...
Write-Host "Canvas performance UI smoke checks passed for $baseUrl"
