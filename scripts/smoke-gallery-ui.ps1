...
function Invoke-PlaywrightCli {
  ...
  $stdout = Join-Path ([System.IO.Path]::GetTempPath()) ("dianshang-gallery-ui-" + $Step + "-out.log")
  $stderr = Join-Path ([System.IO.Path]::GetTempPath()) ("dianshang-gallery-ui-" + $Step + "-err.log")
...
$runnerScript = Join-Path $PSScriptRoot "smoke-gallery-ui-runner.js"

Write-Host "Running gallery UI smoke checks with Playwright session: $session"
$openArgs = @("--yes", "--package", "@playwright/cli", "playwright-cli", "--session", $session, "open", "http://127.0.0.1:3456/?gallery-ui-smoke=open")
...
Write-Host "Gallery UI smoke checks passed"
