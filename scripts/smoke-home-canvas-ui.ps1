...
$session = $env:SMOKE_HOME_CANVAS_UI_SESSION
if (-not $session) {
  $session = "home-canvas-ui-smoke-" + [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds()
}
...
function Invoke-PlaywrightCli {
  ...
  $stdout = Join-Path ([System.IO.Path]::GetTempPath()) ("dianshang-home-canvas-ui-" + $Step + "-out.log")
  $stderr = Join-Path ([System.IO.Path]::GetTempPath()) ("dianshang-home-canvas-ui-" + $Step + "-err.log")
...
$runnerScript = Join-Path $PSScriptRoot "smoke-home-canvas-ui-runner.js"

Write-Host "Running home/canvas UI smoke checks with Playwright session: $session"
$openArgs = @("--yes", "--package", "@playwright/cli", "playwright-cli", "--session", $session, "open", "$baseUrl/?home-canvas-ui-smoke=open")
...
Write-Host "Home/canvas UI smoke checks passed for $baseUrl"
