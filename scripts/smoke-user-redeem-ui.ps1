...
$session = $env:SMOKE_USER_REDEEM_UI_SESSION
if (-not $session) {
...
function Invoke-PlaywrightCli {
  ...
  $stdout = Join-Path ([System.IO.Path]::GetTempPath()) ("dianshang-user-redeem-ui-" + $Step + "-out.log")
  $stderr = Join-Path ([System.IO.Path]::GetTempPath()) ("dianshang-user-redeem-ui-" + $Step + "-err.log")
...
$runnerScript = Join-Path $PSScriptRoot "smoke-user-redeem-ui-runner.js"

Write-Host "Running user redeem UI smoke checks with Playwright session: $session"
...
Write-Host "User redeem UI smoke checks passed"
